...
# process AMASS first
for fpath in tqdm((config.processed_imu_poser / "AMASS").iterdir()):
    # resample to 25 fps
    ...
    # vrot = [_resample(x, target_fps) for x in torch.load(fpath / "vrot_on_vertex.pt")]
    vrot = torch.load(fpath / "vrot_on_vertex.pt")
    vrot = [ torch.from_numpy(v).float() for v in vrot ]
    vrot = [_resample(x, target_fps) for x in vrot ]
...
# process DIP next
for fpath in tqdm((config.processed_imu_poser / "DIP_IMU").iterdir()):
    # resample to 25 fps
    joint = [_resample(x, target_fps) for x in torch.load(fpath / "joint.pt")]
    pose = [math.axis_angle_to_rotation_matrix(_resample(x, target_fps).contiguous()).view(-1, 24, 3, 3) for x in torch.load(fpath / "pose.pt")]
    shape = torch.load(fpath / "shape.pt")
